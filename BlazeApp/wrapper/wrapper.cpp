...
FastTextModel load_fasttext_binary(const std::string &path)
{
    ...
    for (int i = 0; i < num_words; ++i)
    {
        ...
        file.read(reinterpret_cast<char *>(vec.data()), model.vector_size * sizeof(float));
...
PredictionResult predict_sentiment(const char *input_text)
{
    ...
    try
    {
        ...
        static FastTextModel ft_model = load_fasttext_binary("models/maxim_fasttext.bin");
        ...
        static Ort::SessionOptions session_options = []() {
            ...
            // "1" berarti menggunakan allocator lingkungan (mencegah konflik affinity)
...
// Sesuaikan dengan nama file ONNX Anda
#ifdef _WIN32
        static Ort::Session session(env, L"models/maxim-sentiment-models.onnx", session_options);
#else
        static Ort::Session session(env, "models/maxim-sentiment-models.onnx", session_options);
#endif
        ...
        Ort::Value input_tensor = Ort::Value::CreateTensor<float>(memory_info, mean_vec.data(), mean_vec.size(),
